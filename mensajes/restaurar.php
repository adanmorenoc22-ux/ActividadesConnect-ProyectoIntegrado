...
try {
    ...
    if (!$mensaje) {
        ...
        echo json_encode(['success' => false, 'message' => 'No tienes permisos para restaurar este mensaje']);
        exit;
    }
    ...
    $updateQuery = "UPDATE mensajes SET {$campo} = 0 WHERE id = ?";
    $updateStmt = $db->prepare($updateQuery);
    $result = $updateStmt->execute([$mensaje_id]);
    
    if ($result) {
        echo json_encode(['success' => true, 'message' => 'Mensaje restaurado correctamente']);
    } else {
        http_response_code(500);
        echo json_encode(['success' => false, 'message' => 'Error al restaurar el mensaje']);
    }
